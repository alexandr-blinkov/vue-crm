  ...
  <form class="card auth-card" @submit.prevent="submitHandler">
    <div class="card-content">
      ...
      <div class="input-field">
        ...
        <small class="helper-text invalid" v-if="$v.password.$dirty && !$v.password.minLength">Пароль должен быть {{this.$v.password.$params.minLength.min}} символов. {{password.length}}/{{this.$v.password.$params.minLength.min}}</small>
      </div>
      <div class="input-field">
        <input id="email" type="text" v-model.trim="name" :class="{invalid: ($v.name.$dirty && !$v.name.required)}" />
